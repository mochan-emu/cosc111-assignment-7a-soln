...
public class Board extends JPanel implements ActionListener {
    ...
    { 
        ...
        g.fillRect(0, 0, BoardWidth*squareWidth(), 4*squareHeight()+10);
        ...
        if (curPiece.getShape() != Shape.Tetrominoes.NoShape) {
            ...
            for (int j=0; j<nextPiece.length; j++)
            {
            for (int i = 0; i < 4; ++i) {
            	
            	
                int x = j*4 + 1 + nextPiece[j].x(i);
                int y = 20 - nextPiece[j].y(i);
                drawSquare(g, 0 + x * squareWidth() + 10,
                           boardTop + (BoardHeight - y - 1) * squareHeight() - 10,
                           nextPiece[j].getShape());
            }
            }
        }
    }
    ...
    {
    	curPiece.setShape(nextPiece[0].getShape());
    	
    	for (int i=0; i<nextPiece.length-1; i++)
    		nextPiece[i].setShape(nextPiece[i+1].getShape());
        nextPiece[nextPiece.length-1].setRandomShape();
        
        curX = BoardWidth / 2 + 1;
        curY = BoardHeight - 5 + curPiece.minY();
